...
describe("getUiCopy", () => {
  ...
  it("betAmountLabel includes symbol", () => {
    ...
    expect(getUiCopy("zh").betAmountLabel("SOL")).toContain("SOL");
  });
});
...
describe("formatTimeAgoLabel", () => {
  ...
  it("uses compact hour and day labels for older trades", () => {
    expect(formatTimeAgoLabel(Date.now() - 3 * 3_600_000, "en")).toBe("3h ago");
    expect(formatTimeAgoLabel(Date.now() - 2 * 86_400_000, "zh")).toBe("2天前");
  });
