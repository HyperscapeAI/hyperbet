...
const SESSION_SSE_URL = `${API_URL}/api/streaming/session/events`;
const SESSION_POLL_URL = `${API_URL}/api/streaming/session`;
...
export function normalizeCanonicalStreamSession(
  ...
  const rendererHealth =
    ...
    null;
  const authorityHealth = normalizeStreamHealth(
...
export function useCanonicalStreamSession(
  ...
  const applySession = useCallback((nextSession: CanonicalStreamSession) => {
    ...
    applyTimer.current = setTimeout(() => {
      ...
      setIsConnected(true);
    }, sessionPlaybackDelayMs(nextSession));
  }, []);
  ...
  }, [applySession, startFallbackPolling]);
