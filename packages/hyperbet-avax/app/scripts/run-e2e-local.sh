...
EVM_DUEL_ORACLE_ADDRESS="$(jq -r '.evmOracleAddress // empty' "$STATE_PATH")"
if [[ -z "$EVM_DUEL_ORACLE_ADDRESS" || "$EVM_DUEL_ORACLE_ADDRESS" == "null" ]]; then
  echo "[e2e] failed to read evmOracleAddress from $STATE_PATH"
  exit 1
fi
...
write_env_file \
  ...
  AVAX_DUEL_ORACLE_ADDRESS "$EVM_DUEL_ORACLE_ADDRESS" \
