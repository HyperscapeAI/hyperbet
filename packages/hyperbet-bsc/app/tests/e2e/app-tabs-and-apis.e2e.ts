...
test.describe("app tabs and api coverage", () => {
  ...
  test("models surface renders seeded model market data", async ({
    ...
    if (selectedMarket) {
      await expect(page.getByText(selectedMarket.name).first()).toBeVisible();
      await expect(page.getByText(selectedMarket.provider).first()).toBeVisible();
    } else {
      await expect(
        page.getByText("No EVM model markets are indexed yet."),
      ).toBeVisible();
    }
    await page
