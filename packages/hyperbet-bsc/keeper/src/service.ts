...
function toRendererMetricsOverride(
  ...
  const topLevelHlsManifest = asRecord(candidate.hlsManifest);
...
function buildCanonicalStreamSessionFromStreamState(
  ...
  return {
    ...
    emittedAt,
    duelId:
      typeof next.cycle?.duelId === "string" ? next.cycle.duelId : null,
    duelKey: duelKeyHex ? duelKeyHex.replace(/^0x/i, "").toLowerCase() : null,
    phase:
      typeof next.cycle?.phase === "string" ? next.cycle.phase : null,
    phaseVersion:
      typeof next.cycle?.phaseVersion === "number" &&
      Number.isFinite(next.cycle.phaseVersion)
        ? next.cycle.phaseVersion
        : null,
    cycle: next.cycle,
...
function toCanonicalStreamSession(payload: unknown): CanonicalStreamSession | null {
  ...
  const cycle =
    candidate.cycle && typeof candidate.cycle === "object"
      ? (candidate.cycle as Record<string, any>)
      : null;
...
function startKeeperBotIfEnabled(): void {
  ...
  const childEnv = {
    ...
    EVM_KEEPER_CHAINS: process.env.EVM_KEEPER_CHAINS || "bsc",
