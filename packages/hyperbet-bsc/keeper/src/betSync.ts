...
function mergeMarketRecord(
  ...
  return {
    ...
    winner: preferred.winner !== "NONE" ? preferred.winner : fallback.winner,
    syncedAt:
...
export function hasMeaningfulSurface(
  ...
  return Boolean(
    surface.duel.duelKey || surface.duel.duelId || surface.markets.length,
  );
}
...
export function resolveBetSyncBootstrapCursor(params: {
  previousSourceEpoch: number | null;
  nextSourceEpoch: number;
  previousLastAppliedSeq: number;
  latestSeq: number | null;
}): {
  sourceEpochChanged: boolean;
  rebasedLastAppliedSeq: number;
  replayUntilSeq: number | null;
  replayMode: BetSyncReplayMode;
} {
  const sourceEpochChanged =
    params.previousSourceEpoch != null &&
    params.previousSourceEpoch !== params.nextSourceEpoch;
  const rebasedLastAppliedSeq = sourceEpochChanged
    ? 0
    : Math.max(0, params.previousLastAppliedSeq);
  const replayUntilSeq = selectBetSyncReplayUntilSeq({
    resumeSeq: selectBetSyncResumeSeq({
      lastAppliedSeq: rebasedLastAppliedSeq,
    }),
  ...
  return {
    sourceEpochChanged,
    rebasedLastAppliedSeq,
    replayUntilSeq,
    replayMode:
      rebasedLastAppliedSeq > 0 && replayUntilSeq != null ? "replay" : "live",
  };
}
