...
fn q_div(a: i128, b: i128) -> Result<i128> {
    require!(b != 0, PredictionMarketError::MathFixedPointOverflow);
...
fn i256_to_i128(value: I256) -> Result<i128> {
    ...
    require!(value >= min && value <= max, PredictionMarketError::MathFixedPointOverflow);
    Ok(value.as_i128())
}
...
fn q_erf(x: i128) -> Result<i128> {
    ...
    if ax >= four_q {
        return Ok(if is_neg { -Q_ONE } else { Q_ONE });
    }
...
fn q_exp_neg(x: i128) -> Result<i128> {
    if x <= 0 { return Ok(Q_ONE); }

    // For very large x, exp(-x) ≈ 0
    let ten_q = 10i128 << FRAC_BITS;
    if x >= ten_q { return Ok(0); }
...
fn gaussian_cdf_q(z: i128) -> Result<i128> {
...
fn gaussian_pdf_q(z: i128) -> Result<i128> {
    let z2 = q_mul(z, z)?;
...
fn amm_func_q(x: i128, y: i128, l: i128) -> Result<i128> {
    let diff = y - x;
    let z = q_div(diff, l)?;
    let cdf = gaussian_cdf_q(z)?;
    let pdf = gaussian_pdf_q(z)?;

    Ok(q_mul(diff, cdf)? + q_mul(l, pdf)? - y)
}

/// Derivative of invariant w.r.t. x: f'(x) = -Φ(z)
fn func_derivative_q(x: i128, y: i128, l: i128) -> Result<i128> {
    let z = q_div(y - x, l)?;
...
fn get_new_reserve_q(x_guess: i128, y: i128, l: i128) -> Result<i128> {
    ...
    for _ in 0..MAX_NEWTON_ITERS {
        ...
        if q_abs(f) < Q_APPROX {
            let res = q_abs(t);
            return Ok(if res < Q_MIN_RESERVE { Q_MIN_RESERVE } else { res });
        }
        let deriv = func_derivative_q(t, y, l)?;
        require!(deriv != 0, PredictionMarketError::MathConvergenceError);
        t = t - q_div(f, deriv)?;
    }

    err!(PredictionMarketError::MathConvergenceError)
}
...
pub fn get_swap_amount(
    ...
    Ok(q_to_u64(amount_out))
}

pub fn calc_price(x: u64, y: u64, l: u64) -> Result<u64> {
    let rx = u64_to_q(x);
    let ry = u64_to_q(y);
    let lq = u64_to_q(l);

    if lq == 0 { return Ok(500_000); } // Default 50% if no liquidity

    let z = q_div(ry - rx, lq)?;
    let price = gaussian_cdf_q(z)?;

    // Scale to 1_000_000 basis points
    Ok(((price * 1_000_000i128) >> FRAC_BITS) as u64)
}
...
mod tests {
    ...
    fn test_calc_price_equal_reserves() {
        ...
        let price = calc_price(1_000_000, 1_000_000, 1_000_000).unwrap();
        // Allow ±1% tolerance
    ...
    fn test_get_swap_amount_nonzero() {
        let amt_out = get_swap_amount(true, 1_000_000, 1_000_000, 1_000_000, 100_000).unwrap();
    ...
    fn test_swap_symmetry() {
        ...
        let out1 = get_swap_amount(true, 1_000_000, 1_000_000, 1_000_000, 100_000).unwrap();
        let out2 = get_swap_amount(false, 1_000_000, 1_000_000, 1_000_000, 100_000).unwrap();
    ...
    fn test_q_mul_keeps_fractional_bits() {
        ...
        assert_eq!(q_mul(a, b).unwrap(), expected);
    ...
    fn test_q_div_matches_exact_shifted_division() {
        ...
        assert_eq!(q_div(numerator, denominator).unwrap(), expected);
    }
}
