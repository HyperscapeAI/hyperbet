...
async function finalizeAfterDisputeWindow(input: {
  connection: anchor.web3.Connection;
  finalizableAt: number;
  disputeWindowSecs: number;
  finalize: () => Promise<string>;
}): Promise<string> {
  const timeoutMs = Math.max(120_000, (input.disputeWindowSecs + 120) * 1000);
  const deadline = Date.now() + timeoutMs;

  await waitForChainUnixTimestamp(
    input.connection,
    input.finalizableAt,
    timeoutMs,
  );
  ...
  throw new Error(
    `Timed out finalizing after the dispute window at ${input.finalizableAt}`,
  );
}
...
function deriveProposalRecordPda(
  programId: PublicKey,
  duelKey: readonly number[],
...
export class SolanaProgramRuntime {
  ...
  static async create(
    ...
    const clobIdl = JSON.parse(
      readFileSync(validator.assets.duelMarket.idlPath, "utf8"),
    ) as anchor.Idl;

    const fightProgram = new anchor.Program(fightIdl, provider) as Program<any>;
    const clobProgram = new anchor.Program(clobIdl, provider) as Program<any>;
  ...
  async ensureOracleReady(
    ...
    const existingConfig = await (
      this.fightProgram.account as any
    ).oracleConfig.fetchNullable(oracleConfig);
  ...
  async ensureClobConfig(options: {
    ...
    const existingConfig = await (
      this.clobProgram.account as any
    ).marketConfig.fetchNullable(config);
  ...
  async reportResult(args: {
    ...
    const oracleConfig = deriveOracleConfigPda(this.fightProgram.programId);
    const duelState = deriveDuelStatePda(
      this.fightProgram.programId,
      args.duelKey,
    );
    ...
    const duelStateAccount = await this.fetchDuelState(duelState);
    const duelStartTs = Number(duelStateAccount.duelStartTs);
    await waitForChainUnixTimestamp(this.connection, duelStartTs);
    const now = await currentChainUnixTimestamp(this.connection);
    if (now === null) {
      throw new Error(
        "Confirmed Solana block time is unavailable; refusing to report a result",
      );
    }
    const duelEndTs = args.duelEndTs ?? now;
    if (duelEndTs < duelStartTs || duelEndTs > now) {
      throw new Error(
        `Invalid result time ${duelEndTs}; expected ${duelStartTs} <= duelEndTs <= ${now}`,
      );
    }
    const replayHash = [...hashLabel(`${args.seed}:replay`)];
    const resultHash = [...hashLabel(`${args.seed}:result`)];
    ...
    const proposalSignature = await this.fightProgram.methods
      .proposeResult(
        ...
        toBn(
          BigInt(`0x${Buffer.from(hashLabel(args.seed)).toString("hex")}`) %
            10_000n,
        ),
        replayHash,
        resultHash,
    ...
    const config = await (this.fightProgram.account as any).oracleConfig.fetch(
      oracleConfig,
    );
    const disputeWindowSecs = Number(config.disputeWindowSecs);
    const proposedDuelState = await this.fetchDuelState(duelState);
    const pendingProposedAt = Number(proposedDuelState.pendingProposedAt);
    const finalizableAt = pendingProposedAt + disputeWindowSecs;
    const finalizeSignature = await finalizeAfterDisputeWindow({
      connection: this.connection,
      finalizableAt,
      disputeWindowSecs,
      finalize: () =>
        this.fightProgram.methods
