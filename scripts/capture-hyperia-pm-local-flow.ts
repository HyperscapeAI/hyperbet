...
const hyperiaUiUrl = requiredEnv("HYPERIA_UI_URL");
...
const hyperiaReadySelector =
  process.env.CAPTURE_HYPERIA_READY_SELECTOR ||
...
async function recordEvent(
  ...
  await takeScreenshot(
    hyperiaUiUrl,
    `${prefix}.hyperia.png`,
    hyperiaReadySelector,
  ).catch((error) => {
    console.error(
      `[pm-local:capture] hyperia screenshot failed for ${rawLabel}: ${String(error)}`,
    );
    throw error;
  });
  await takeScreenshot(
