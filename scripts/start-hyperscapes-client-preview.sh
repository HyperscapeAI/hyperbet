...
require_env() {
  ...
  local value="${!name:-}"
  if [[ -z "$value" ]]; then
    echo "[hyperscapes-client] missing required env: $name" >&2
    exit 1
  fi
}
...
require_env HYPERSCAPES_PUBLIC_CDN_URL
require_env GAME_CLIENT_PORT

NODE_BIN="${NODE_BIN:-$(command -v node)}"
DUEL_CLIENT_NODE_BIN="${DUEL_CLIENT_NODE_BIN:-$NODE_BIN}"
CLIENT_DIR="$HYPERSCAPES_ROOT/packages/client"
CLIENT_DIST="$CLIENT_DIR/dist"

if [[ ! -f "$CLIENT_DIST/stream.html" ]]; then
  echo "[hyperscapes-client] missing built preview assets at $CLIENT_DIST/stream.html" >&2
  exit 1
fi
