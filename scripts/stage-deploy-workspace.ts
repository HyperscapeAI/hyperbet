...
type KeeperTarget = "keeper:solana";
...
function parseTarget(argv: string[]): KeeperTarget {
  ...
  throw new Error(
    "unsupported SOL-only deployment target; expected --target=keeper:solana",
  );
}
...
function stagePackageRootManifest(
  relativeDir: string,
...
function stageForTarget(target: KeeperTarget) {
  const keeperDir = path.join(rootDir, "packages/hyperbet-solana/keeper");
  const packageRoot = "packages/hyperbet-solana";
  ...
  stagePackageRootManifest(packageRoot, workspacePackagesRoot);
...
function writeWorkspaceLockfile(
  ...
  const tempRoot = mkdtempSync(
    path.join(os.tmpdir(), "hyperbet-deploy-workspace-"),
  );
  try {
    ...
    cpSync(path.join(rootDir, "bun.lock"), path.join(tempRoot, "bun.lock"), {
      force: true,
    });
    cpSync(path.join(workspacePackagesRoot, "packages"), tempPackagesRoot, {
      recursive: true,
      force: true,
    });

    const keeperPackageJson = path.join(
      rootDir,
      packageRoot,
      "keeper",
      "package.json",
    );
    const tempKeeperPackageJson = path.join(
      tempRoot,
      packageRoot,
      "keeper",
      "package.json",
    ...
    cpSync(
      path.join(tempRoot, "bun.lock"),
      path.join(keeperDir, "workspace.bun.lock"),
      { force: true },
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `failed to generate staged workspace lockfile for ${packageRoot}: ${message}`,
    );
  } finally {
